...
public class NewsDao {
      public boolean  add(News news, int[] catids){
       ...
       if(con!=null){
        try{
            ...
            String sql = "Insert into news(title, description,posted_on,image,posted_by,status,status_text)values(?,?,?,?,?,?,?)";
            PreparedStatement smt = con.prepareStatement(sql);
            smt.setString(1, news.getTitle());
            smt.setString(2, news.getDescription());
            smt.setString(3, news.getPosted_on());
            smt.setString(4, news.getImage());
            smt.setInt(5, news.getPosted_by());
       ...
       if(con!=null){
        try{
            ...
            if(smt.executeUpdate()>0) {
               for(int cat_id : catids)
               {
                   sql = "update ncr set cat_id = ? where news_id=?";
                   smt = con.prepareStatement(sql);
  ...
  public int  getCountByReporter(int reporterid){
       ...
       if(con!=null){
        try{
            String sql = "select count(*) from news where posted_by=?";
            PreparedStatement smt = con.prepareStatement(sql);
